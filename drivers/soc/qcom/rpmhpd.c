...
struct rpmhpd_freq_map {
	struct rpmhpd *pd;
	unsigned long freq[RPMH_ARC_MAX_LEVELS];
};
...
static const struct of_device_id rpmhpd_match_table[] = {
	{ .compatible = "qcom,rpmhpd-sdm845", .data = &sdm845_desc },
	{ }
};
...
static int rpmhpd_send_corner(struct rpmhpd *pd, int state, unsigned int corner)
{
	...
	return rpmh_write(pd->rpmh_client, state, &cmd, 1);
};
					   ...
					   struct dev_pm_opp *opp)
{
	...
	of_property_read_u32(np, "qcom-corner", &corner);
	of_node_put(np);
...
static int rpmhpd_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < num; i++) {
		...
		rpmhpds[i]->rpmh_client = rpmh_client;
		...
		rpmhpds[i]->pd.get_performance_state = rpmhpd_get_performance;
