...
static int __clk_rcg2_configure(struct clk_rcg2 *rcg, const struct freq_tbl *f)
{
	...
	if (clk_hw_is_prepared(hw)) {
		ret = genpd_clk_prepare_vote_rate(&rcg->clkr, f->freq);
		if (ret) {
...
out:
	if (clk_hw_is_prepared(hw))
		...
		genpd_clk_unprepare_vote_rate(&rcg->clkr, old_rate);
...
static int clk_rcg2_prepare(struct clk_hw *hw)
{
	...
	return genpd_clk_prepare_vote_rate(&rcg->clkr, rate);
}
...
static void clk_rcg2_unprepare(struct clk_hw *hw)
{
	...
	genpd_clk_unprepare_vote_rate(&rcg->clkr, rate);
}
