...
static int gcc_late_init_genpd(void)
{
	...
	pr_err("GP1 disable unprepare 200000000\n");
	clk_disable_unprepare(gcc_gp1_clk.clkr.hw.clk);

	pr_err("===========================================\n\n");
	pr_err("clock rate change for gcc_cpuss_ahb_clk_src on cx\n");
	clk_set_rate(gcc_cpuss_ahb_clk_src.clkr.hw.clk, 50000000);
